...
Este capítulo apresenta a fundamentação teórica, que consiste no estudo das técnicas e algoritmos utilizados. O capitulo divide-se em três seções. A seção \ref{sec:segmentacao} apresenta técnicas de subtração de fundo, limiarização, operações morfológicas, esqueletonização e análise de componentes principais. A seção \ref{sec:descricao}  por sua vez apresenta propriedades de regiões em imagens e descritores de \emph{fourier}. Finalmente a seção \ref{sec:classificacao} apresenta uma visão geral de redes neurais e métodos de treinamento.
...
\subsection{Operações morfológicas} \label{sub:fundteor_op_morfologicas}
...
\item Elemento estrutural é uma mascara de pixeis que é utilizada para fazer operações sobre uma imagem. Cada elemento estrutural possui um pixel de origem. A figura \ref{fig:fundteor_elementos_estruturais} mostra exemplos de elementos estruturais e suas respectivas origens.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{./figs/elementos_estruturais.png}
	\caption{Exemplos de elementos estruturais.}
	\fonte{Autoria própria}
	\label{fig:fundteor_elementos_estruturais}
\end{figure}

\item O elemento estrutural pode ser utilizado para fazer operações sobre uma imagem binária. Considere por exemplo a operação no conjunto $A$, feita pelo elemento estrutural $B$ (figuras \ref{fig:fundteor_elementos_estruturais_ex_A} e \ref{fig:fundteor_elementos_estruturais_ex_B}): Crie um novo conjunto $C$ sobrepondo $B$ em $A$ da maneira que a origem de $B$ passe  por todos os pixeis de $A$. Em cada ponto que a origem de $B$ passou verifique se $B$ está completamente contido em $A$. Se estiver marque o ponto no conjunto $C$. A figura \ref{fig:fundteor_elementos_estruturais_ex_C} mostra o resultado da operação, ou seja, o conjunto $C$.

\begin{figure}[H]
        \centering
	\begin{subfigure}[b]{0.4\textwidth}
                \includegraphics[width=\textwidth]{./figs/elementos_estruturais_A.png}
                \caption{Imagem binária $A$}
                \label{fig:fundteor_elementos_estruturais_ex_A}
        \end{subfigure}
       ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc.
          %(or a blank line to force the subfigure onto a new line)
        \begin{subfigure}[b]{0.4\textwidth}
                \includegraphics[width=\textwidth]{./figs/elementos_estruturais_B.png}
                \caption{Elemento estrutural $B$}
                \label{fig:fundteor_elementos_estruturais_ex_B}
        \end{subfigure}
        %add desired spacing between images, e. g. ~, \quad, \qquad etc.
        %(or a blank line to force the subfigure onto a new line)
        
	\begin{subfigure}[b]{0.4\textwidth}
                \includegraphics[width=\textwidth]{./figs/elementos_estruturais_C.png}
                \caption{$C$, resultado da operação de $B$ em $A$}
                \label{fig:fundteor_elementos_estruturais_ex_C}
        \end{subfigure}

        \caption{Imagem binária, elemento estrutural e operação de $B$ em $A$}
	\fonte{Autoria própria}
	\label{fig:elementos_estruturais_ex}
\end{figure}

\end{itemize}

A seguir apresenta-se as operações de erosão e de dilatação, que são necessárias para as operações de abertura e fechamento que são apresentadas na sequencia.


\subsubsection{Erosão}

Sendo $A$ e $B$ conjuntos no espaço $Z^2$, a erosão do conjunto $A$ pelo elemento estrutural $B$, denotada por $A \ominus B$, é definida na equação \ref{eq:fundteor_erosao} \cite{GONZALEZ-2006}.
...
Pode-se ver melhor esse processo na figura \ref{fig:fundteor_erosao_A_B}, onde o conjunto A é erodido pelo elemento estrutural B formando o conjunto $A \ominus B$.
...
% redes neurais
\subsection{Método de \emph{BR}}
% trainbr
\subsection{Método de \emph{LM}}
% trainlm
...
\cite{BELLATOR}. O trabalho desda figura \ref{fig:diagsis} ilustra a configuração do projeto.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{./figs/diagsis.png}
	\caption[Diagrama do projeto de oficina 3]{Diagrama do projeto de oficina 3.}
...
jhgfvytf \cite{BELLATOR} disponível em mídia digital. A caixa continha os seguintes itens:
...
O m fio ou implementação de stream de vídeo, motivo principal para a utilização desse PC no projeto Bellator~\cite{BELLATOR}, a equipe optou por descartar esse recurso e utilizar outra placa, descrita em detalhes na seção \ref{sec:espts7260}. O processo de reconstrução e adaptação da plataforma robótica é descrito em detalhes no capítulo \ref{chap:desenv}.
