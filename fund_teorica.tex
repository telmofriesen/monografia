...
%TODO: introdução a segmentacao de imagens
...
%TODO: introdução a descrição de imagens

\subsection{Propriedades de regiões}
% propriedades de regiões
\subsection{Descritores de \emph{fourier}}

Considere a figura \ref{}. Os pixeis da imagem podem ser representados como sendo pares de coordenadas $(x_{0},y_{0}),(x_{1},y_{1}),(x_{2},y_{2}),...,(x_{K-1},y_{K-1})$, onde $K$ é o numero de pixeis na imagem. Além disso os pixeis podem ser representados como uma sequencia $x(k) = x_{k}$ e $y(k) = y_{k}$ ou como $s(k) = \left \[ s(k), y(k)\right \]$ para $k = 0,1,2,...,K-1$. Além disso pode-se tratar as coordenadas dos pixeis como números complexos: $s(k) = x(k) + j y(k)$.
