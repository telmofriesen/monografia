...
Para \emph{perceptrons} na camada de saída $Q$ os pesos $w(k+1)$ podem ser calculado pela equação \ref{eq:fundteor_nn_perceptron_error_multi_desc_2}.

\begin{equation}
w(k+1) = w(k) + \alpha (r_{j}-O_{j}) h'_{j}(I_{j}) O_{k}
...
\begin{itemize}
\item $w(k+1)$ são os pesos atualizados do \emph{perceptron} $q$;
\item Para uma matriz $nxn$ existem $n$ autovetores \cite{AUTOVETORES};
\item Todos os autovetores são ortogonais entre si \cite{GONZALEZ-2006}.
\end{itemize}
...
Os coeficientes $w(k+1)$ para um \emph{perceptron} na camada $J$, que é precedido por uma camada $K$, podem ser determinados pela equação \ref{eq:fundteor_nn_perceptron_error_multi_desc_2}.
...
w(k+1) = w(k) + \alpha  h'_{j}(I_{j}) O_{k} (\sum_{p=1}^{N_{P}}\delta _{P}\omega _{jp})
