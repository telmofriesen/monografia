...
Este capítulo apresenta a o desenvolvimento do projeto, detalhando cada parte do sistema desenvolvido. A descrição apresentada a seguir referencia os conceitos apresentados no capítulo \ref{chap:fundteor}. 

A figura \ref{fig:desenv_overview} apresenta uma visão geral do sistema desenvolvido.
...
O capitulo divide-se em oito seções. As primeiras seis seções apresentam os blocos da figura \ref{fig:desenv_overview}: A seção \ref{sec:desenv_seg1} e a seção \ref{sec:desenv_seg2} mostram como os conceitos da seção \ref{sec:segmentacao} foram aplicados para segmentar a imagem capturada. A seção \ref{sec:desenv_desc1} e a seção \ref{sec:desenv_desc2} apresentam como os descritores de \emph{fourier} e as propriedades estudados na seção \ref{subsec:propriedades} foram aplicados para formar um descritor para as imagens. As seções \ref{sec:desenv_class1} e \ref{sec:desenv_class2} apresentam como os descritores foram utilizados para classificar e determinar o ângulo das imagens.
Na seção \ref{sec:desenv_class3} apresenta-se o processo de treinamento do sistema e ao final, na seção \ref{sec:desenv_consideracoes}, algumas considerações sobre o desenvolvimento do projeto.
...
A segunda etapa da segmentação é responsável por normalizar a imagem. Esta etapa recebe como entrada a região de interesse e apresenta como saída duas opções de normalização da imagem, além de um autovetor e seu oposto. O processo pode ser visto na figura \ref{fig:desenv_seg2}.
...
\section{Classificação: etapa 1} \label{sec:desenv_class1}
...
Para essa tarefa foi utilizada uma rede neural, que é detalhada mais a frente na seção \ref{sec:desenv_class3}.
%TODO: detalhar a rede

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{./figs/classificacao_1.jpg}
	\caption{Primeira etapa da classificação.}
	\fonte{Autoria própria}
...
A segunda etapa da descrição recebe como entrada a imagem normalizada pelo autovetor correto. A função desta etapa é gerar um descritor para a imagem normalizada, que auxiliará na segunda etapa de classificação para determinação do ângulo do objeto.

Tanto esta etapa, quanto a classificação descrita na próxima seção, seriam desnecessárias em ambientes ideais. Veja por exemplo as imagens da figura \ref{fig:desenv_desc3}. Todas as imagens foram obtidas para um mesmo objeto, sujeito às mesmas condições gerais, mas em rotações diferentes. Mudanças pequenas, como ruído ou pequenas diferenças na iluminação, influenciaram a direção do autovetor calculado produzindo resultados diferentes.
...
A diferença entre o autovetor calculado para cada caso é tratada utilizando uma segunda rede neural, específica para cada classe. O descritor obtido nessa etapa portanto deve conter informações relacionadas à posição do objeto na imagem. Utilizou-se portanto para essa tarefa um descritor contendo 10 descritores de \emph{fourier}, conforme representado na figura \ref{fig:desenv_desc2}.
...
\section{Classificação: etapa 2} \label{sec:desenv_class2}

Finalmente a última etapa recebe como entrada um descritor da imagem normalizada, a classe a qual pertence o objeto e o autovetor utilizado na normalização. O objetivo dessa etapa é fornecer o angulo de rotação do objeto na imagem.

Considere a figura \ref{fig:desenv_class3}. Os eixos $x,y$ são as coordenadas da imagem. O eixo $x'$ é o eixo que foi assumido como sendo o zero para o objeto da figura. Pela figura tem-se então a equação \ref{eq:desenv_beta}.
...
\item $k$ é a diferença entre o angulo do autovetor e o zero do objeto. Valor este que pode variar de normalização para normalização conforme visto na figura \ref{fig:desenv_desc3}.
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{./figs/classificacao_3.jpg}
	\caption{Representação do autovetor (em vermelho), eixos da imagem (em amarelo) e eixo do objeto (em azul).}
	\fonte{Autoria própria}
	\label{fig:desenv_class3}
\end{figure}

É necessário portanto determinar o valor $k$ para cada imagem normalizada. Utilizou-se para essa tarefa uma segunda rede neural. Essa rede recebe como entrada um descritor da imagem normalizada e informa na saída o valor de $k$. Para cada classe é gerada uma rede diferente, visto que as variações são diferentes de classe para classe. Após obter o valor $k$ da rede basta utilizar a equação \ref{eq:desenv_beta} para determinar o angulo $\beta$ do objeto.
...
\section{Classificação: treinamento e topologia} \label{sec:desenv_class3}

Pode-se utilizar de maneira genérica os descritores de \emph{fourier} para esse caso. Como cada classe tem sua própria rede para cálculo dos desvios do autovetor pode-se optar por uma rede simples 
