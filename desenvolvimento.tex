...
Este capítulo apresenta a o desenvolvimento do projeto, detalhando cada parte do sistema desenvolvido. A descrição apresentada a seguir referencia os conceitos apresentados no capítulo \ref{chap:fundteor}. A figura \ref{fig:desenv_overview} apresenta uma visão geral do sistema desenvolvido.
...
A segunda etapa da descrição recebe como entrada a imagem normalizada pelo autovetor correto. A função desta etapa é gerar um descritor para a imagem normalizada, que auxiliará na etapa de previsão para determinação do ângulo do objeto.

Tanto esta etapa, quanto previsão descrita na próxima seção, seriam desnecessárias em ambientes ideais. Veja por exemplo as imagens da figura \ref{fig:desenv_desc3}. Todas as imagens foram obtidas para um mesmo objeto, sujeito às mesmas condições gerais, mas em rotações diferentes. Mudanças pequenas, como ruído ou pequenas diferenças na iluminação, influenciaram a direção do autovetor calculado produzindo resultados diferentes.
...
A diferença entre o autovetor calculado para cada caso é tratada na próxima etapa. O descritor obtido nessa etapa portanto deve conter informações relacionadas à posição do objeto na imagem. Sendo assim utilizou-se para essa tarefa um descritor contendo 10 descritores de \emph{fourier}, conforme representado na figura \ref{fig:desenv_desc2}.
...
Para formar o conjunto de treinamento das redes é necessário que se obtenha amostras de imagens com classes e ângulos conhecidos.  Em seguida basta executar todos os passos até obter os descritores de entrada das redes e calcular o valor de $k$ para o treinamento da segunda rede.
